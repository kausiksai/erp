"""PID-based file lock with stale-PID detection.

Mirrors `email_automation/mailbox/lockfile.py`. Kept here so the OCR
package has no cross-package dependency.

Semantics:
    * `acquire()` writes the current PID into the lock file.
    * If a lock file already exists, the recorded PID is checked. Live
      PID -> LockError. Dead PID -> stale, reclaimed automatically.
    * `release()` removes the lock if this process owns it.
    * Use as a context manager: `with FileLock(path): ...`.

Cross-platform PID liveness check — Win32 OpenProcess on Windows,
